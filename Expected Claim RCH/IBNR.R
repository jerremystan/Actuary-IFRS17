...
ifrs_cf <- rbind(lrc_1, lrc_2, lrc_3, lrc_4, lrc_5
                 , lrc_6
                 , lrc_7
)

# var_1 <- read.csv("input csv/variable_all_runs_2023_202312_1.csv")
...
# 
# var_cf <- rbind(var_1, var_2, var_3, var_)
# 
...
ibnr_initial_cf <- ifrs_cf[(ifrs_cf$variable_name == "CURRENT CLAIM LIC OS" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 110) 
                   | (ifrs_cf$variable_name == "CURRENT CHE LIC" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 110)
                   | (ifrs_cf$variable_name == "PAST CLAIM LIC OS" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 102) 
                   | (ifrs_cf$variable_name == "PAST CHE LIC" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 102),]

ibnr_initial_cf$`Sum Total` <- rowSums(ibnr_initial_cf[, 10:ncol(ibnr_initial_cf)], na.rm = TRUE)

ibnr_YE_cf <- ifrs_cf[(ifrs_cf$variable_name == "CURRENT CLAIM LIC OS" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 134) 
                           | (ifrs_cf$variable_name == "CURRENT CHE LIC" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 134)
                           | (ifrs_cf$variable_name == "PAST CLAIM LIC OS" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 134) 
                           | (ifrs_cf$variable_name == "PAST CHE LIC" & ifrs_cf$custom_dim_1 == "IBNR" & ifrs_cf$run_number == 134),]

ibnr_YE_cf$`Sum Total` <- rowSums(ibnr_YE_cf[, 10:ncol(ibnr_YE_cf)], na.rm = TRUE)
...
writeData(wb, sheet = "Initial", ibnr_initial_cf)
writeData(wb, sheet = "YE", ibnr_YE_cf)

saveWorkbook(wb, "LIC OS_IBNR.xlsx", overwrite = TRUE)
